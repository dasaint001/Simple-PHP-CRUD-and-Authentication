...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($input_code)){
        $code_err = "Please enter a course code.";     
    } else{
        $code = $input_code;
    }
    ...
    if(empty($input_unit)){
        $unit_err = "Please enter the course code.";     
    } elseif(!ctype_digit($input_unit)){
        $unit_err = "Please enter an integer value.";
    } else{
        $unit = $input_unit;
    }
    
    // Check input errors before inserting in database
    if(empty($name_err) && empty($code_err) && empty($unit_err)){
        // Prepare an update statement
        $sql = "UPDATE courses SET name=?, code=?, unit=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssi", $param_name, $param_code, $param_unit, $param_id);
